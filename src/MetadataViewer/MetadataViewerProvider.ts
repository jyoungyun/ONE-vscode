...
export class MetadataViewerProvider
  implements vscode.CustomReadonlyEditorProvider<RelationViewerDocument>
{
  ...
  public static register(context: vscode.ExtensionContext): void {
    ...
    const registrations = [
      vscode.window.registerCustomEditorProvider(
        MetadataViewerProvider.viewType,
        provider,
        {
          webviewOptions: {
  ...
  ): Promise<RelationViewerDocument> {
    const document: RelationViewerDocument =
      await RelationViewerDocument.create(uri);
  ...
  async resolveCustomEditor(
    document: RelationViewerDocument,
